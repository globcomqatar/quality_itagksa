// Inward Serial No dropdown — populates from the linked Job Card.
frappe.ui.form.on('Quality Inspection', {
	...
	on_submit(frm) {
		...
		if (ref_type !== 'Job Card') return;
		frappe.show_alert({
			message: __('Returning to {0} {1}', [__(ref_type), ref_name]),
			indicator: 'green',
		}, 3);
		frappe.model.clear_doc(ref_type, ref_name);
		frappe.set_route('Form', ref_type, ref_name);
	},
});
